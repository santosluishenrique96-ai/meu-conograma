...
async function getSchedulePreferencesForUser(userId: string): Promise<SchedulePreferencesRow | null> {
...
export async function listDiaryEntries(
  ...
  if (offset !== undefined && (!Number.isFinite(offset) || !Number.isInteger(offset) || offset < 0)) {
...
export async function upsertDiaryEntry(
  ...
  const entryDate = normalizeISODate(date);
  const dateForSnapshot = typeof date === "string" ? new Date(entryDate) : date;
  ...
  if (existingEntry) {
    ...
    if (validated.perceived_result !== undefined) updatePayload.perceived_result = validated.perceived_result;
    ...
    if (validated.evolution_photo_id !== undefined) updatePayload.evolution_photo_id = validated.evolution_photo_id;
    if (scheduledFocusSnapshot !== null) {
      updatePayload.scheduled_focus_snapshot = scheduledFocusSnapshot as unknown as import("@/integrations/supabase/types").Json;
    }
  ...
  const insertPayload: DiaryEntryInsert = {
    ...
    scheduled_focus_snapshot: scheduledFocusSnapshot as unknown as import("@/integrations/supabase/types").Json | null,
  };
